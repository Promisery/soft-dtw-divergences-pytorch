...
def sdtw_C(C, gamma=1.0, return_all=False):
  ...
  V = torch.zeros((size_X + 1, size_Y + 1), device=C.device, dtype=C.dtype)
  V[:, 0] = 1e10
  V[0, :] = 1e10
  V[0, 0] = 0

  # Tensor containing the probabilities of transition.
  P = torch.zeros((size_X + 2, size_Y + 2, 3), device=C.device, dtype=C.dtype)
...
def sdtw_grad_C(P, return_all=False):
  ...
  """
  E = torch.zeros((P.shape[0], P.shape[1]), device=P.device, dtype=P.dtype)
  E[-1, :] = 0
  E[:, -1] = 0
  E[-1, -1] = 1
  P[-1, -1] = 1
...
def sdtw_directional_derivative_C(P, Z, return_all=False):
  ...
  V_dot = torch.zeros((size_X + 1, size_Y + 1), device=P.device, dtype=P.dtype)
  V_dot[0, 0] = 0
...
def sdtw_hessian_product_C(P, E, V_dot):
  ...
  P_dot = torch.zeros((E.shape[0], E.shape[1], 3), device=P.device, dtype=P.dtype)
...
def squared_euclidean_cost(X, Y, return_all=False, log=False):
  ...
  XY = torch.matmul(X, Y.T).to(X_sqnorms)
  ...
  if return_all:
    ...
    C_XX = -torch.matmul(X, X.T)
    C_XX += X_sqnorms[:, None]
    C_XX += X_sqnorms

    C_YY = -torch.matmul(Y, Y.T)
    C_YY += Y_sqnorms[:, None]
    C_YY += Y_sqnorms
